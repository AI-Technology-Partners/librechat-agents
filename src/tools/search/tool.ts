...
import { z } from 'zod';  // AITP: ADD
import type * as t from './types';
import {
  WebSearchToolDescription,
  WebSearchToolName,
  countrySchema,
  enginesSchema, // AITP: ADD
  imagesSchema,
  languageSchema, // AITP: ADD
  videosSchema,
  querySchema,
  dateSchema,
  newsSchema,
...
async function executeParallelSearches({
  ...
  date?: DATE_RANGE;
  country?: string;
  language?: string; //AITP: ADD
  engines?: string; //AITP: ADD
...
function createTool({
  ...
  // schema: Record<string, unknown>; // AITP: REMOVE
  schema: z.ZodObject<Record<string, z.ZodTypeAny>>; // AITP: ADD
  ...
  return tool(
    async (rawParams, runnableConfig) => {
      ...
      // const { query, date, country: _c, images, videos, news } = params; // AITP: REMOVE
      const { query, date, country: _c, language, engines, images, videos, news } = params;  // AITP: ADD
...
export const createSearchTool = (
  ...
  //   videos: videosSchema,
  //   news: newsSchema,
  //   language: languageSchema, // AITP: ADD
  //   engines: enginesSchema, // AITP: ADD
  // };
  // AITP: REMOVE - END

  // AITP: ADD - START
  const schemaShape: Record<string, z.ZodTypeAny> = {
    query: z.string().describe(querySchema.description),
    date: z.enum(Object.values(DATE_RANGE) as [DATE_RANGE, ...DATE_RANGE[]]).optional(),
    images: z.boolean().optional().describe(imagesSchema.description),
    videos: z.boolean().optional().describe(videosSchema.description),
    news: z.boolean().optional().describe(newsSchema.description),
    language: z.string().optional().describe(languageSchema.description),
    engines: z.string().optional().describe(enginesSchema.description),
  };

  schemaShape.date = schemaShape.date.describe(dateSchema.description);
  // AITP: ADD - END
  ...
  // AITP: ADD - START
  if (searchProvider === 'serper') {
    schemaShape.country = z.string().optional().describe(countrySchema.description);
  }
  // AITP: ADD - END

  const toolSchema = z.object(schemaShape);
